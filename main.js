...
const results = document.getElementById("listingg");
const u = new Audio('uuu.ogg');
const lancer = new Audio('splat.ogg');
var animate = true;
...
async function start() {
  document.getElementById("listingg").innerHTML = "";
  section = "greetmsg";
...
async function btndisplay(choice) {
  if (section !== choice) {
    ...
    // back button func
    const item = document.createElement("li");
    item.appendChild(document.createElement("span")).textContent = "Back";
    item.setAttribute("onmousedown", `start()`);
    results.appendChild(item);
